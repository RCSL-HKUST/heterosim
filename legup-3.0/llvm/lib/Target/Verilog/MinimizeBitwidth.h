...
class triStateMask {
    ...
    public:
        ...
        //64 1s
        triStateMask (Value *inst,int maxBitwidth,bool maxIsPositive, bool isPositive);
        triStateMask ();
        void print ();
...
class MinimizeBitwidth : public FunctionPass {
    public:
        ...
        uint8_t getMSB(Value *I){ return triStateMap[I].MSB;}
        uint8_t getLSB(Value *I){ return triStateMap[I].LSB;}
        uint8_t getExtendFrom(Value *I){ return triStateMap[I].extendFrom;}
        uint8_t getBitwidth(Value *I){ return triStateMap[I].bitwidth;}
        bool bitwidthIsKnown(Value *I) { if(triStateMap.find(I)==triStateMap.end()) return false;
                                         return (triStateMap[I].extendFrom != 127);}
        uint8_t getMinBitwidth(Value *I) { 
            assert(triStateMap.find(I) != triStateMap.end() && "Value not found in triStateMap\n");
            if(LEGUP_CONFIG->getParameterInt("NO_MIN_BITWIDTH")) 
                return (triStateMap[I].bitwidth);
            else
                return (triStateMap[I].extendFrom+1);
        }
        bool isSigned(Value *I) {
            assert(triStateMap.find(I) != triStateMap.end() && "Value not found in triStateMap\n");
    ...
    private:
        ...
        std::map<Value *, triStateMask> triStateMap;
