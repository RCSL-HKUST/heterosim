...
RTLSignal::RTLSignal() : bitwidth(RTLWidth()), nativeBitwidth(RTLWidth()), driver(0), defaultDriver(0), isSigned(false) {}
RTLSignal::RTLSignal(std::string name, std::string value, RTLWidth bitwidthIn, 
                     RTLWidth nativeBitwidthIn, bool isSigned)
    : name(name), value(value), bitwidth(bitwidthIn), driver(0),
    defaultDriver(0), isSigned(isSigned)
{
    if(nativeBitwidthIn.str()=="" && bitwidthIn.str()!="") {
        nativeBitwidth=bitwidthIn;    
    }
    else nativeBitwidth=nativeBitwidthIn;
}
...
void RTLSignal::connect(RTLSignal *s, const Instruction *I) {
...
RTLWidth RTLSignal::getNativeWidth() const {
    return nativeBitwidth;
}
void RTLSignal::setWidth(RTLWidth w) {
    bitwidth = w;
    if(nativeBitwidth.str()=="") nativeBitwidth=w;
}


void RTLSignal::setNativeWidth(RTLWidth w) {
    nativeBitwidth = w;
}
RTLModule::~RTLModule() {
...
void RTLSignal::modifySignalWithMBW(MinimizeBitwidth *MBW, Value *instr) {
    if(MBW->bitwidthIsKnown(instr)) {
        this->setWidth(RTLWidth(MBW->getMinBitwidth(instr)));
        this->setSigned(MBW->isSigned(instr));
    }
}
...
RTLSignal *RTLModule::addIn(std::string name, RTLWidth width) {
    return addIn(name,width,width,false);
}
RTLSignal *RTLModule::addIn(std::string name, RTLWidth width, 
                            RTLWidth nativeBitwidth, bool isSigned) {
    RTLSignal *in = new RTLSignal(name, "", width,nativeBitwidth,isSigned);
    in->setType("input");
    ports.push_back(in);
    return in;
}
...
RTLSignal *RTLModule::addReg(std::string name, RTLWidth width) {
    return addReg(name,width,width,false);
}
RTLSignal *RTLModule::addReg(std::string name, RTLWidth width, 
    ...
    if (exists(name)) {
        ...
        //if (width) {
            s->setWidth(width);
            s->setNativeWidth(nativeBitwidth);
            s->setSigned(isSigned);
//            if(isSigned) errs() << "set sign to true\n";
//            else errs() << "set sign to false\n";
        //}
        return s;
    }
    RTLSignal *s = new RTLSignal(name, "", width,nativeBitwidth,isSigned);
//    if(isSigned) errs() << "2set sign to true\n";
//    else errs() << "2set sign to false\n";
    signals.push_back(s);
    s->setType("reg");
    return s;
}
...
RTLSignal *RTLModule::addWire(std::string name, RTLWidth width) {
    return addWire(name,width,width,false);
}
...
RTLSignal *RTLModule::addOut(std::string name, RTLWidth width, RTLWidth nativeBitwidth, bool isSigned) {
    RTLSignal *out = new RTLSignal(name, "", width,nativeBitwidth,isSigned);
    out->setType("output");
    ports.push_back(out);
    return out;
}
RTLSignal *RTLModule::addOut(std::string name, RTLWidth width) {
    return addOut(name,width,width,false);
}

RTLSignal *RTLModule::addOutReg(std::string name, RTLWidth width) {
    return addOutReg(name,width,width,false);
}
RTLSignal *RTLModule::addOutReg(std::string name, RTLWidth width, 
                                RTLWidth nativeBitwidth, bool isSigned) {
    RTLSignal *out = new RTLSignal(name, "", width, nativeBitwidth, isSigned);
...
void RTLModule::verifyConnection(const RTLSignal *signal, const Allocation *alloc) const {
    for (unsigned i = 0; i < signal->getNumDrivers(); ++i) {
        ...
        if(!LEGUP_CONFIG->getParameterInt("MB_MINIMIZE_HW")) {
            if (signal->getNativeWidth().numBits(this, alloc) !=
                    driver->getWidth().numBits(this, alloc)) {
                errs() << "Incompatible width. Signal width: " <<
                signal->getWidth().numBits(this, alloc) << " Driver width: " <<
                driver->getWidth().numBits(this, alloc) << "\n";
