...
// Helper functions for isInstructionSharable
bool share_div (Instruction *I) {
    return (isDiv(I) && LEGUP_CONFIG->getParameterInt("SHARE_DIV"));
}

bool share_rem (Instruction *I) {
    return (isRem(I) && LEGUP_CONFIG->getParameterInt("SHARE_REM"));
}
...
bool is_floating_point (Instruction *I) {
    return (
...
void BipartiteWeightedMatchingBinding::CheckAllWereAssigned(int
    ...
    assert(numOperationsWithAssignment == numOperationsToShare);
}
