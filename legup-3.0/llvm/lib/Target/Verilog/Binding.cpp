...
Binding::Binding(Allocation *alloc, FiniteStateMachine *fsm,
            Function *Fp) : alloc(alloc), fsm(fsm), Fp(Fp) {
    LVA = alloc->getLVA(Fp);
    assert(LVA);
    MBW = alloc->getMBW(Fp);
    assert(MBW);
...
) 
{
    ...
    if (!Intersection.empty()) {
        ...
        if (lastState.find(BB) == lastState.end()) {
...
{
    for (Value::use_iterator use = i1->use_begin(),
            use_end = i1->use_end(); use != use_end; ++use) {
    ...
    for (Value::use_iterator use = i2->use_begin(),
            use_end = i2->use_end(); use != use_end; ++use) {
        Instruction * successor = dyn_cast<Instruction> ( *use );
        if (!successor) continue; // successor is not an instruction
