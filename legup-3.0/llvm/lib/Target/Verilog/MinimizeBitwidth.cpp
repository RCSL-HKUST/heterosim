...
void triStateMask::max(triStateMask a,triStateMask b)
{
    ...
    extendFrom = std::max(a.extendFrom,b.extendFrom);
...
triStateMask* MinimizeBitwidth::getOp(Instruction *inst, int op_index, int64_t maxVal, 
    ...
    if(triStateMap.find(inst->getOperand(op_index)) == triStateMap.end())
    {
        ...
        triStateMap[inst->getOperand(op_index)]=triStateMask(inst->getOperand(op_index),
                                                maxBitwidth,maxIsPositive,isPositive);
    }
    ...
    if(out)
    {
        *out=NULL;
        if(triStateMap.find(inst) == triStateMap.end())
        {
            ...
            triStateMap[inst]=triStateMask(inst,maxBitwidth,maxIsPositive,isPositive);
        }
...
bool MinimizeBitwidth::backwardPropagate(Instruction *inst){
    ...
    if(found) {
        ...
        if(*in==in_temp) return false;
        //sanity check
/*        if((in->extendFrom < in->MSB) && in->MSB < (in->bitwidth-1))
            error = "BackwardPropagate: Signed number with one or more leading zero\n";
        if(in->LSB>0 && triStateMask::pow2(in->LSB-1)&(in->bitIsDynamic|in->bitValue)) 
            error = "backwardPropagate: LSB-1 is dynamic or static 1\n";
        if(in->MSB>0 && !(triStateMask::pow2(in->LSB)&(in->bitIsDynamic|in->bitValue)))
            error = "backwardPropagate: LSB is static 0\n";
        //uint128_t temp0 = !(in->bitIsDynamic|in->bitValue);
        if(in->MSB!=0 && (!((in->bitIsDynamic|in->bitValue)&triStateMask::pow2(in->MSB)))) error = "backwardPropagate: MSB is 0\n";
        //if(in->MSB!=0 && (temp0&triStateMask::pow2(in->MSB))) error = "backwardPropagate: MSB is 0\n";
        if(in->extendFrom<in->LSB) error="backwardPropagate: extendFrom is < LSB\n";
        if(in->LSB > in->MSB) error="backwardPropagate: LSB > MSB\n";
        if(!in->bitIsDynamic && in->MSB!=in->extendFrom) 
            error="backwardPropagate: extendFrom < MSB when not dynamic\n";
        if(error!="")
        {
            printValue(inst);
            printf("\n%s",error.c_str());
...
triStateMask MinimizeBitwidth::forwardPropagateSDiv(triStateMask *out, triStateMask *op0, triStateMask *op1) {
    triStateMask out_temp = *out;
    uint8_t minShift = (int)(log2(op1->minPossibleVal().lower()));
    if(op1->MSB==op1->bitwidth-1) out_temp.MSB=out_temp.bitwidth-1;
    out_temp.LSB=0;
    out_temp.bitIsDynamic = triStateMask::biggestVal(out_temp.MSB+1);
    out_temp.bitValue = 0; //don't bother with the bitValue
    out_temp.extendFrom = op0->extendFrom-minShift;
...
//                if(dyn_cast<Instruction>(inst)->getOpcode()==Instruction::Add)
...
void MinimizeBitwidth::printStats(Function &F) {
        ...
        itr1 != totalBits.end();++itr1,++itr2,++itr3,++itr4)
    {
        ...
        if(name!="INVALID")
        {
            ...
            printf("%s %lu %lu %f %lu %f %lu %f\n",name.c_str(),itr1->second,itr4->second,
                (double)itr4->second/itr1->second,itr2->second,
                (double)itr2->second/itr1->second,itr3->second,(double)itr3->second/itr1->second);
        }
        else
        {
    ...
    printf("%lu %lu %f %lu %f %lu %f \n",total_bits,total_bits_eliminated,
        (double)total_bits_eliminated/total_bits,total_high_bits_eliminated,
        (double)total_high_bits_eliminated/total_bits,total_low_bits_eliminated,
...
bool MinimizeBitwidth::runOnFunction(Function &F) {
    ...
    if(range_filename!="") populateRangeMap(range_filename); 
    ...
    bool minBitwidthOff = LEGUP_CONFIG->getParameterInt("NO_MIN_BITWIDTH");
	...
	if(~minBitwidthOff && (maxNumPasses==-1 || numPasses<maxNumPasses)) {
		for (Function::iterator BB = F.begin(), EE = F.end(); BB != EE; ++BB) {
		    for (BasicBlock::iterator I = BB->begin(), E = BB->end(); I != E; ++I) {
			Instruction *inst = dyn_cast<Instruction>(I);
			if (!inst) continue;
			if(backwardPropagate(inst)) changed_in_back_propagation=true;
		    }
		}
	}
	numPasses++;
...
void rewriteIR(Instruction *inst, Module *Mod, unsigned minWidth, Value *reg,
    ...
    Instruction *trunc = CastInst::CreateTruncOrBitCast(reg, newWidthType,
            "min_width", inst);
