...
)
{
    ...
    // and ending with graphs of size 1.
...
void create_dot_directory_for_function (string foldername) {
    int prevent_warnings;
    if ( LEGUP_CONFIG->getParameterInt("WRITE_TO_DOT") ||
         LEGUP_CONFIG->getParameterInt("WRITE_TO_VERILOG")
        )
    {
...
) 
{
    ...
    while (!Graphs.empty()) {
        if ( Graphs.front()->size() >=
             LEGUP_CONFIG->getParameterInt("MAX_SIZE") )
            break; // stop growing
    ...
    while (!Nodes.empty()) { // while still nodes to visit
        ...
        for (int predecessor_number=0; predecessor_number < 2; ++predecessor_number)
        {
            ...
            // and add the copy to our data structures
                ...
                if (!check_predecessor_legality(pred, g, BB, this->MBW, this->fsm)) {
                    continue;
                }
...
) 
{
    ...
    string name = LEGUP_CONFIG->getOpNameFromInst(pred);
...
bool sharing_is_enabled_for_this_instruction(Instruction *I) {
    ...
    if (isAdd(I)) {
        return LEGUP_CONFIG->getParameterInt("SHARE_ADD");
    }
    if (isSub(I)) {
        return LEGUP_CONFIG->getParameterInt("SHARE_SUB");
    }
    if (isShift(I)) {
        return LEGUP_CONFIG->getParameterInt("SHARE_SHIFT");
    }
    if (isBitwiseOperation(I)) {
        return LEGUP_CONFIG->getParameterInt("SHARE_BITOPS"); 
    }
    return false;
}
...
) 
{
    ...
    unsigned minwidth = LEGUP_CONFIG->getParameterInt("MIN_WIDTH");
...
//
// Moreover, two instructions must have bit-widths within BIT_DIFF_THRESHOLD 
...
{
    int threshold = LEGUP_CONFIG->getParameterInt("BIT_DIFF_THRESHOLD");
...
)
{
         ...
         (numPairs >= LEGUP_CONFIG->getParameterInt("FREQ_THRESHOLD")) )
    {
        ...
        if (LEGUP_CONFIG->getParameterInt("WRITE_TO_VERILOG")) {
...
)  
{
    ...
    string ErrorInfo;
    raw_fd_ostream Out(full_filename.c_str(), ErrorInfo);
