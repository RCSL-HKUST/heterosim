...
void ByRef(int *a, int b, volatile int thread_id) {
    printf("thread_id: %d\n", thread_id);
    *a = *a + b;
}
...
int main() {
    ...
    int *SHARED_MEM_global = malloc_shared(num_threads*sizeof(int), &global);
    ...
    for (i = 0; i<max; i++) {
        ...
        // accelerator call
        for (j = 0; j < num_threads; j++) {
            struct args tmp = {&SHARED_MEM_global[j], increment[j], j};
