...
void generate_set(int xstart, int xend, int xmax, int xmin, int ymax, int ymin,
        int xres, int yres, int itr, char *ptr, volatile int thread_id)
{
...
void* pthread_generate_set(void *data)
{
    ...
    xend = arg->xres >> arg->dividend;
    row_size = xend / arg->num_threads;
...
#ifdef SW_ONLY
#define OFFSET (i*xend*3*arg->yres)
#else
#define OFFSET (0)
#endif

    for (i = 0; i < loop_itr; i++) {
        generate_set(i*xend + arg->thread_id * row_size, i*xend + (arg->thread_id+1) * row_size, arg->xmax, arg->xmin, arg->ymax, arg->ymin, arg->xres, arg->yres, arg->itr, arg->SHARED_MEM_ptr + 3*arg->yres*arg->thread_id*row_size + OFFSET, arg->thread_id);

        // post-copy
        memcpy_from_shared(arg->ptr+ 3*arg->yres*(i*xend + arg->thread_id*row_size), arg->SHARED_MEM_ptr + 3*arg->yres*arg->thread_id*row_size, 3*arg->yres*row_size);
    }
...
void generate_mandelbrot(int itr, int xres, int yres, IplImage *img)
{
    ...
    // pre-allocate
    char *SHARED_MEM_ptr = malloc_shared((3*xres*yres) >> dividend, ptr);

    pthread_t pool[NUM_THREADS];
    struct args arg_array[NUM_THREADS];

    for (i = 0; i < NUM_THREADS; i++) {
        struct args arg_element = {dividend, xmax, xmin, ymax, ymin, itr, xres, yres, ptr, SHARED_MEM_ptr, i, NUM_THREADS};
...
int main()
{
  ...
  for (i = MIN_ITER; i <= MAX_ITER; i+= STEP) {
    generate_mandelbrot(i, 1000, 1000, img);
  }
