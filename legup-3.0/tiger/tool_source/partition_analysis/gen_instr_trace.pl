...
while (<SRC>) {
	# lines in func_table.src: <00800e18 g     F .text	00000028 legup_seq_accel_function>
	chop;
	my @tmp = split(/\s+/);
	if ( scalar(@tmp) != 6 ) {
...
exists $hybrid_func_addr->{"legup_seq_$acel_func_name"} or die "Error: Hyprid Wrapper legup_seq_$acel_func_name not found.\n";
...
#		print "  "x scalar(@func_stack) ."On Call: ".$gxemul_func_name->{$curr_func_addr};	printf (" - %08x\n", $addr);
		...
		if ($depth == 1) { # Just entering accelerating function
			...
			for (my $i = 0;	$i< $hybrid_func_size->{"legup_seq_$acel_func_name"}; $i+=4) {
				$tmp = sprintf ("%08x", $hybrid_func_addr->{"legup_seq_$acel_func_name"} + $i);
				print DST $tmp."\n";
			}
