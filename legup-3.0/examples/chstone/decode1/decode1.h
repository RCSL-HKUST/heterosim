...
decode_block (int comp_no, int *out_buf, int *HuffBuff)
{
  ...
  p_quant_tbl =
    &p_jinfo_quant_tbl_quantval[(int)p_jinfo_comps_info_quant_tbl_no[comp_no]*DCTSIZE2+DCTSIZE2];
...
void decode_start()//int x[4000],int y[4000])
//int decode_start(int *x, int m, int *y, int n)
{
  ...
  int *out_data_image_width = OutData_image_width;
  int *out_data_image_height = OutData_image_height;
  int *out_data_comp_vpos = OutData_comp_vpos;
  int *out_data_comp_hpos = OutData_comp_hpos;
	  ...
	  for (i = 0; i < RGB_NUM; i++)
	    {
	      WriteBlock (&rgb_buf[0][i][0],
			  ...
			  &out_data_comp_hpos[i], &OutData_comp_buf[i*BMP_OUT_SIZE]);
	    }
	  CurrentMCU++;
	}
