...
int decode_start(int x[4000],int m, int y[4000], int n)
//int decode_start(int *x, int m, int *y, int n)
{   
    FILE *input;
  FILE *output; 
    int i;
  ...
  int *out_data_image_width = &OutData_image_width_;
  int *out_data_image_height = &OutData_image_height_;
  int *out_data_comp_vpos = OutData_comp_vpos_;
  int *out_data_comp_hpos = OutData_comp_hpos_;
   ...
   */
  *out_data_image_width = p_jinfo_image_width_;
  *out_data_image_height = p_jinfo_image_height_;
	  ...
	  for (i = 0; i < RGB_NUM; i++)
	    {
	      WriteBlock (&rgb_buf[0][i][0],
			  ...
			  &out_data_comp_hpos[i], &OutData_comp_buf_[i][0]);
	    }
	  CurrentMCU++;
	}
