     ...
     int full_pel_vector;	/* MPEG-1 only */
{
  ...
  *((volatile unsigned int *)0xf000f00C) = 20;  

  a = decode_motion_vector (buff, m, &PMV[0],n);

//  fprintf(stderr,"{");
//  for(i=0;i<1;i++)
//  {fprintf(stderr,"%d ",PMV[0]);}
//  fprintf(stderr,"}\n");
  //PMV[0]=buff[0];
  if (dmv)
  ...
  *((volatile unsigned int *)0xf000f008) = (unsigned int)(buff1);
  *((volatile unsigned int *)0xf000f00C) = 20;  

  //fprintf(stderr,"{");
  //for(i=0;i<5;i++)
  //{fprintf(stderr,"%d ",buff1[i]);}
  //fprintf(stderr,"}\n");
  a = decode_motion_vector (buff1, m, &PMV[1],n);
  //PMV[1]=buff1[0];
  //fprintf(stderr,"{");
  //for(i=0;i<1;i++)
  //{fprintf(stderr,"%d ",PMV[1]);}
  //fprintf(stderr,"}\n");
