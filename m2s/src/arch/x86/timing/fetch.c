...
static struct x86_uop_t *X86ThreadFetchInst(X86Thread *self,
	...
	while (list_count(x86_uinst_list)) {
		...
		if (uop->flags & X86_UINST_MEM) {
			if (uinst->address == ctx->mem_mod_low)
            	{ 

                  mem_read_copy(ctx->mem, uinst->address, 4, &(uop->data));
                  
                 ctx->mem_low = uop->data-(uop->data & (self->data_mod->block_size-1));
            	 uop->addr = uinst->address; 
            	}
            else if (uinst->address == ctx->mem_mod_high)
                 { 
                   mem_read_copy(ctx->mem, uinst->address, 4, &(uop->data));
            ...
            else if (uinst->address == ctx->flush_ba)
            	{ 
            	  mem_read_copy(ctx->mem, uinst->address, 4, &(uop->data));
            	  //fprintf(stderr, "%x, low\n", uinst->address);
            	   
                   //fprintf(stderr, "ba %x ", uop->data);
                   //fprintf(stderr, "ba %x ", self->data_mod->block_size);
                 ctx->flush_base = uop->data-(uop->data & (self->data_mod->block_size-1));
                 ctx->flush_size = uop->data & (self->data_mod->block_size-1);
                   //fprintf(stderr, "ba_final %x ", ctx->flush_base);
                   //fprintf(stderr, "ba_final %x\n", ctx->flush_size);
            	 uop->addr = uinst->address; 
            	}
            else if (uinst->address == ctx->flush_si )
                 { 
                 mem_read_copy(ctx->mem, uinst->address, 4, &(uop->data));
                   //fprintf(stderr, "si %x ", ctx->flush_size);
                   //fprintf(stderr, "si %x ", uop->data);
                   //fprintf(stderr, "si %x ", self->data_mod->block_size);
                  ctx->flush_size = ((ctx->flush_size + uop->data-1)& (~(self->data_mod->block_size-1)))+ self->data_mod->block_size;
                   //fprintf(stderr, "si_final %x\n", ctx->flush_size);
                   uop->addr = uinst->address;
                 }        	
				...
				if (self->standalone) {
                   ...
                   // if  (uop->addr>=0xbffeff00 && uop->addr<=0xbffeff60)
                  //fprintf(stderr, "%x,%x\n", uop->addr, uop->data );
