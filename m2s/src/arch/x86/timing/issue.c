...
static int X86ThreadIssueSQ(X86Thread *self, int quantum)
{
	...
	while (!linked_list_is_end(sq) && quantum)
	{
        ...
        if ((store->addr == store->ctx->mem_mod_low)||(store->addr == store->ctx->mem_mod_high)||(store->addr==store->ctx->flush_ba)||(store->addr==store->ctx->flush_si))
	   	{ X86ThreadRemoveFromSQ(self);
	   	  if(store->addr==store->ctx->flush_si)
	   	  {
	   	  	// fprintf(stderr, "%x %d\n", store->ctx->flush_base, store->ctx->flush_size);
	   	     mem_flush(self->data_mod, store->ctx->flush_base, store->ctx, store->ctx->flush_size);	
...
static int X86ThreadIssueLQ(X86Thread *self, int quant)
{
	...
	while (!linked_list_is_end(lq) && quant)
	{
		...
		if ((load->addr == load->ctx->mem_mod_low)||(load->addr == load->ctx->mem_mod_high)||(load->addr==load->ctx->flush_ba)||(load->addr==load->ctx->flush_si))
