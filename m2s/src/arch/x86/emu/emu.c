...
void X86EmuLoadContextsFromConfig(X86Emu *self, struct config_t *config, char *section) {
	...
	ctx->mem_mod_low = config_read_int(config, section, "MemModLow", 0xF000F000);
	ctx->mem_mod_high = config_read_int(config, section, "MemModHigh", 0xF000F004);
	ctx->flush_ba = config_read_int(config, section, "FlushBase", 0xF000F008);
	ctx->flush_si = config_read_int(config, section, "FlushSize", 0xF000F00C);

    mem_map(ctx->mem, ctx->mem_mod_low, 16 , mem_access_read | mem_access_write);
