...
int EV_FPGA_MEM_LOAD_PRE_FINISH;

int EV_FPGA_MEM_STORE_PRE;
...
int EV_FPGA_MEM_STORE_PRE_FINISH;
...
void mod_handler_nmoesi_load(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_LOAD_ACTION)
	{
        ...
        //if(stack->addr == 0x2f04 && stack->state)
        //	fprintf(stderr, "%s \n", mod->name );
...
void mod_handler_nmoesi_store(int event, void *data)
{
	...
	{   
       ...
       if(stack->uop) 
       {
        //fprintf(stderr, "mem addr  %x and %x\n", stack->uop->ctx->mem_low,stack->uop->ctx->mem_high);
        //fprintf(stderr, "loadfinish  %x and %x at %lld\n", stack->addr,stack->uop->addr,esim_time);
       	if(stack->uop->addr>=stack->uop->ctx->mem_low && stack->uop->addr<=stack->uop->ctx->mem_high)
...
//        {
//          mod_low = mod_get_low_mod(mod_low, stack->addr);
//        } 

           
           mod_stack_id++;
           //fprintf(stderr, "flush  %x at %lld\n", stack->addr,esim_time);
           new_stack = mod_stack_create( mod_stack_id,
		      mod, stack->uop->addr, ESIM_EV_NONE, NULL, 0);
	       new_stack->event_queue = NULL;
	       new_stack->event_queue_item = NULL;
	       new_stack->client_info = NULL;
	       new_stack->size = 4;
	       new_stack->uop = stack->event_queue_item;
	       new_stack->ctx = stack->uop->ctx;

           esim_schedule_event(EV_MOD_FLUSH, new_stack, 0);  
        }
    }
...
void fpga_mem_load_handler (int event, void *data)
{
    ...
    {   unsigned int offset;
		...
		new_stack = mod_stack_create(stack->id, mod, addr,
			EV_FPGA_MEM_LOAD_PRE_FINISH, stack, latency_add);
        new_stack->buf = buf;
	    ...
	    new_stack->ctx = ctx; 
	    new_stack->task = stack->task;
	    new_stack->load = stack->load;



		esim_schedule_event(EV_FPGA_MEM_LOAD, new_stack, lat); 

		esim_schedule_event(EV_FPGA_MEM_LOAD_PRE, stack, inter);
		...
		//fprintf(stderr, "%d, %d, %d\n", chunksize, stack->size, stack->transfer_count );
    ...
    if (event == EV_FPGA_MEM_LOAD_PRE_FINISH)
    {   
		mem_debug("%lld %lld 0x%x %s load\n", esim_time, stack->id,
			stack->addr, mod->name);
		mem_trace("mem.new_access name=\"A-%lld\" type=\"load\" "
...
void fpga_mem_store_handler (int event, void *data)
{
    ...
    if (event == EV_FPGA_MEM_STORE_PRE)
    {   unsigned int offset;
    	int chunksize; 
		...
		new_stack = mod_stack_create(stack->id, mod, addr,
			EV_FPGA_MEM_STORE_PRE_FINISH, stack, latency_add);
        new_stack->buf = buf;
	    new_stack->event_queue = stack->event_queue;
	    new_stack->event_queue_item = stack->event_queue_item;
	    new_stack->client_info = stack->client_info;
		...
		esim_schedule_event(EV_FPGA_MEM_STORE, new_stack, lat); 

		esim_schedule_event(EV_FPGA_MEM_STORE_PRE, stack, inter);
    ...
    if (event == EV_FPGA_MEM_STORE_PRE_FINISH)
    {   
		mem_debug("%lld %lld 0x%x %s load\n", esim_time, stack->id,
			stack->addr, mod->name);
		mem_trace("mem.new_access name=\"A-%lld\" type=\"load\" "
...
void fpga_mem_large_load_handler (int event, void *data)
{
	...
	//	   fprintf(stderr,"mem_large_load,%x,%d\n",stack->addr,stack->size);
		...
		esim_schedule_event(EV_FPGA_MEM_LARGE_LOAD_START, new_stack, latency); 
	...
	{   
        ...
        esim_schedule_event(EV_FPGA_MEM_LARGE_LOAD_ACTION, stack, 0); 
...
void fpga_mem_large_store_handler (int event, void *data)
{
	...
	if (event == EV_FPGA_MEM_LARGE_STORE)
	{
		...
		esim_schedule_event(EV_FPGA_MEM_LARGE_STORE_START, new_stack, latency); 
	...
	{   
        ...
        esim_schedule_event(EV_FPGA_MEM_LARGE_STORE_ACTION, stack, 0); 
...
void mod_handler_flush(int event, void *data)
{
	struct mod_stack_t *stack = data;
	struct mod_stack_t *new_stack;
	...
	ctx->flush = ctx->flush+1;

	//fprintf(stderr,"cpu1:%d-%lld ",ctx->flush,esim_time); 


    phy_addr = mmu_translate( ctx->address_space_index, addr);
    mod_low = mod;
        while(mod_low->kind != mod_kind_main_memory) 
        {
          mod_low = mod_get_low_mod(mod_low, phy_addr);
        } 
     ...
     for(i=0;i < size/4;i++)
        { 
          phy_addr = mmu_translate( ctx->address_space_index,
           ...
           mod_stack_id++;
           new_stack = mod_stack_create( mod_stack_id,
		      mod_low, phy_addr, EV_MOD_FLUSH_FINISH, stack,0);
...
//            fprintf(stderr,"%d ",i);
 ...
 //       fprintf(stderr,"a ");
        ...
        {	
        	mod_stack_return(stack);
        	ctx->flush = ctx->flush-1;
        //    fprintf(stderr,"cpu2:%d-%lld ",ctx->flush,esim_time);  
    //        fprintf(stderr,"\n");  
		    return;
        }
