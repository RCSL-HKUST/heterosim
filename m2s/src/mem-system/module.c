...
long long mem_flush(struct mod_t *mod, unsigned int addr, X86Context *ctx, int size)
{
	struct mod_stack_t *stack;
	int event;
   ...
   //     } 
	...
	//	 		addr + i*4);
	       ...
	       stack->ctx = ctx;
         //  fprintf(stderr, "%d\n", stack->size );
           esim_schedule_event(EV_MOD_FLUSH, stack, 0);   
     //   } 
